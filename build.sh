...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/koson-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    KOSOND_EXEC=kosond.exe
fi
...
get_tag
popd
echo "var GUI_KOSON_VERSION = \"$TAGNAME\"" >> version.js
...
$QMAKE ../koson-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy kosond to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$KOSON_DIR/bin/$KOSOND_EXEC $BIN_PATH
fi
